...
import { serviceFaqData, FaqItem } from "@/data/serviceFaqData";

interface ServiceFaqProps {
  service: FaqItem["service"] | FaqItem["service"][];
}
...
const ServiceFaq: React.FC<ServiceFaqProps> = ({ service }) => {
  ...
  const filteredFaqs = serviceFaqData.filter((item) => {
                    ...
                    {isOpen && (
                      ...
                      >
                        <div className="pl-[48px] md:pl-[72px] pr-2 md:pr-12 pt-2 pb-4">
                          <p className="text-slate-600 leading-relaxed text-[13px] md:text-sm font-sans font-normal max-w-3xl whitespace-normal break-words">
                            {item.answer}
                          </p>
                        </div>
                      </motion.div>
